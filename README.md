...
## Соответствие критериям задания (чек-лист статуса)
...
*  **Дополнительно (опции готовы, но не включено по умолчанию):**

  * Логирование в файл запросов/батчей — легко добавить (структура кода готова)
...
* ➕ **Дополнительно (опции готовы, но не включено по умолчанию):**
  ...
  * Расширенный лог действий в файл — можно включить быстро
...
*  **Обязательная часть (10б):**
  ...
  * REST API:
    ...
    * `POST /train` — обучение **TF-IDF + One-Class SVM** на **не-alert** строках (`alert_tag == '-'`)
...
*  **Обязательная часть (5б):**
  ...
  * API сценариев использования:
    ...
    * `POST /scenario/train_model` — обучение на non-alert подмножестве
...
* ➕ **Дополнительно (пока не реализовано):**
  ...
  * Аутентификация/авторизация (готово к добавлению, не включено)
  * Разделение сценариев по ролям (можно добавить на уровне роутов)
...
*  **Дополнительно (частично):**
  ...
  * История сценариев и дашборды по метрикам — **не реализованы**, архитектура позволяет добавить
...
* **Данные:** сырые `.log` строки из BGL (LogHub); первый токен строки — alert-тэг. `'-'` трактуется как **не-alert** (норма), остальные — alert.
* **Подход:** обучаем **One-Class SVM** на **не-alert** строках как на «норме». Признаки — **TF-IDF** по тексту сообщения (n-граммы 1–2). На инференсе получаем метку (1 — норма, -1 — аномалия) и decision score.
